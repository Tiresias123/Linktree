...
| États | `aria-expanded="true"` : fond `--fond-surface-2`, chevron pivoté de 180° en `var(--mvt-rapide)` 120 ms. Burger ouvert : croix (translations ±6 px, rotations ±45°). Fermeture par clic sur le déclencheur, clic extérieur ou `Échap`, laquelle **rend le focus au déclencheur** (`actio.js` l. 90-94). Sélecteurs : `aria-pressed`. |
...
| Accessibilité | Hiérarchie `h1.vh` (l. 179) → `h2.vh` « À la une » → `h3` titre majeur / `h3` « Le fil réglementaire » → `h4` titres de brèves. Le visuel est un `role="img"` porteur de l'`aria-label` « Schéma : ligne de partage tracée par le TMF entre gestion de fonds d'autrui et vente d'information. », le `<svg>` intérieur étant `aria-hidden` : le texte alternatif énonce la démonstration, pas la forme. |
...
|---|---|---|
| Consentement exprès, **jamais pré-coché** | `<input type="checkbox" name="consentement">` sans attribut `checked` | `index.html` l. 770 |
| Case distincte de l'envoi | `<label class="infolettre__consentement">` séparé du `div.infolettre__form` | l. 769-778 |
| Finalités énoncées à la collecte | « Je consens expressément à recevoir l'infolettre *Actio Dispatch* et les communications électroniques commerciales d'Actio, conformément à la **Loi canadienne anti-pourriel (LCAP)**. » | l. 772-774 |
| Droit de retrait | « Ce consentement peut être retiré en tout temps par le lien de désabonnement présent dans chaque message. » | l. 774-775 |
| Renvoi aux politiques | Liens « politique de confidentialité » et « conditions d'utilisation » en `--encre-accent` | l. 775-776 |
| Blocage sans consentement | « Le consentement exprès est requis par la Loi canadienne anti-pourriel (LCAP). » puis `consent.focus()` | `actio.js` l. 190-196 |
| Preuve du consentement | « En production : double opt-in, horodatage du consentement et journalisation de la preuve (art. 13 LCAP). » | `actio.js` l. 198-201 |
...
**Pas de widget de prix clignotant.** Aucune animation hors les 120 à 220 ms de transition des états de survol, neutralisées sous `prefers-reduced-motion` (`actio.css` l. 10-15). Un cours qui clignote fabrique de l'urgence ; l'urgence est l'outil du vendeur.
...
| D5 | `--encre-hausse`, `--encre-baisse`, `--encre-stable` orphelins depuis le passage à `--encre-*`, avec un appel résiduel en style en ligne (l. 65) | Supprimer la section 10 de `tokens.css` et corriger la l. 65, ou rétablir l'usage |
| D6 | ~~Le déclencheur du mega-menu est un `<a>` porteur d'`aria-expanded` dont le clic est annulé~~ — **partiellement corrigé** : c'est désormais un `<button type="button">` (l. 96-100). Il reste qu'aucun lien ne mène à l'index de la rubrique 1 | Ajouter un `<a href="#regulation">` adjacent au bouton, qui ne porte plus que le chevron |
...
| **D10** | ~~`.une__visuel` : dégradé bâti sur `--actio-bleu-palais` et `--actio-turquoise-fonce`, libellés blancs à 3,0:1 et 1,8:1 en sombre~~ — **corrigé** : `actio.css` l. 485-491 pose `linear-gradient(135deg, var(--degrade-1), var(--degrade-2), var(--degrade-3))`, invariants par thème (14,4 / 11,1 / 6,4:1 sous du blanc) | — |
...
| D18 | **Partiellement corrigé** : `.infolettre .btn--primaire` (l. 632) est passé à `var(--encre-accent-texte)`. Subsiste `.infolettre .btn--primaire:hover` (l. 633), qui pose `background: #fff` et `border-color: #fff` : valeurs brutes | `var(--fill-primaire-texte)` |

**Arbitrages ouverts.** (1) **L'indicateur de conformité doit-il survivre au défilement ?** Trois options : dupliquer une pastille compacte dans `.entete__actions` ; rendre `.cotations` collant sous `.entete`, au prix d'environ 40 px de hauteur permanente ; assumer la disparition. À trancher avant le gel de la maquette. (2) **Méthode de calcul du temps de lecture** : aucune formule n'est fixée — mots par minute, traitement des tableaux et des schémas, arrondi —, de sorte que « 11 min » reste un chiffre sans auteur. (3) **État d'erreur du flux de cotations en production** : dernière valeur horodatée, tiret, ou disparition du bandeau ? (4) **Sort du lien « registre de vérification »** quand le bandeau de probité disparaîtra au lancement ; proposition : huitième entrée de la colonne Transparence. (5) **Le commentaire « Hauteur fixe 36 px »** de `index.html` l. 42 est inexact — la hauteur mesurée est d'environ 40 px et n'est fixée nulle part : corriger le commentaire ou poser un `min-height`. (6) ~~**Le composant `.etiquette`** n'existe pas~~ — **créé**. Étiquette éditoriale neutre : police à chasse fixe, filet tireté, aucune couleur de statut, aucune pastille, pas de casse haute. Elle ne peut pas être confondue avec un badge statutaire. Les six emplois décoratifs que le prototype faisait des badges — « Démo », « Analyse », « Dossier », « Guide » — y ont été reversés. (7) **Les sept pages de gouvernance et le registre de vérification n'existent pas** : le pied de page promet une transparence qu'il ne livre pas — le seul reproche auquel ce projet ne peut pas survivre.
