...
4. **Le pré-en-tête est une seconde accroche, jamais une redite.** Il porte le **mécanisme** quand l'objet porte la **conséquence**. Cible **90 à 120 caractères** ; l'édition nº 001 est à 115. Implémenté par `{{PREENTETE}}` dans `chassis.html` : `<div>` masqué suivi d'un bourrage de caractères invisibles qui empêche l'aspiration du corps. **Interdits :** « Voir dans le navigateur », « Ouvrez pour découvrir », toute reprise de l'objet.
...
4. **Le champ `objet`.** `tools/emails.mjs` ne contrôle ni la longueur de l'objet, ni l'absence d'émoji, ni la distinction `objet` / `titre` : trois contrôles à ajouter, sans quoi la doctrine du §1.3 n'est qu'une intention.
